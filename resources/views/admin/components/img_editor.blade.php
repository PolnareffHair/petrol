...
<div  class="sort_editor"  >
                ...
                <div >
                    ...
                    <div class="top_buttons">
                        ...
                        <button class="refresh_sort" id="{{$save_id}}">
...
<script>
        $(document).ready(function() {
            ...
            document.getElementById('{{$sorter_name}}').addEventListener('click', function(event) {
                ...
                if (DeletedButton) {
                    ...
                    $.ajax({
                        ...
                        complete: function() {StopLoading('#product_operation');}
                    });
                }
            });
            ...
            //set order img 
            document.getElementById('{{$save_id}}').addEventListener('click', function(event) {

            StartLoading("#product_operation");

                $('#{{$add_id}}').fadeTo("0.15", 0.33);
                $('#{{$save_id}}').fadeTo("0.15", 0.33);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#{{$add_id}}').attr("disabled", false)
                        $('#{{$save_id}}').attr("disabled", false)
                        $('#{{$add_id}}').fadeTo("0.15", 1);
                        $('#{{$save_id}}').fadeTo("0.15", 1);
                    },
                    error: function(xhr, status, error) {
                        console.log(error);
                        get_img_editor();
